...
// コンストラクタ
//=========================================================
CMyParticle::CMyParticle(int nPriority) : CObject(nPriority),
m_pos(VECTOR3_NULL),
m_col(COLOR_WHITE),
m_nLength(NULL),
m_nMaxParticle(NULL),
...
// 生成処理
//=========================================================
CMyParticle* CMyParticle::Create
(
...
{
	// インスタンス生成
	CMyParticle* pParticle = new CMyParticle;
...
void CMyParticle::Update(void)
{
	...
	for (int nCntApper = 0; nCntApper < m_nMaxParticle; nCntApper++)
	{
		...
		// エフェクト生成
		CMyEffect* pEffect = CMyEffect::Create(pos, col, Move, nLife, fRadius);
